...
import com.example.demo.entity.ReturnDto;
...
public class GlobalExceptionHandler {
    ...
    public ReturnDto indexOutOfBoundsExceptionHandler(IndexOutOfBoundsException ex) {
//        return resultFormat(6, ex);
        return new ReturnDto(false,"数组越界异常",ex.getMessage());
    }

    @ExceptionHandler(NoHandlerFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ReturnDto noHandlerFoundHandler(NoHandlerFoundException ex) {
        System.out.println("请求地址错误");
        System.out.println(ex.getHeaders());
        System.out.println(ex.getHttpMethod());
        System.out.println(ex.getRequestURL());
        return new ReturnDto(false, "404错误(请求地址不存在)",ex.getMessage());
    }
